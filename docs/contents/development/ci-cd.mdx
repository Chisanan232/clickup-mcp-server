...
3. **Documentation** (`Docusaurus`):
   ...
   📖 **For detailed documentation deployment architecture, see [Documentation Deployment](#documentation-deployment)**
...
The documentation workflow (`documentation.yaml`) handles building and deploying project documentation using **dual trigger mechanisms** to ensure reliable deployment after releases.
...
on:
  push:
    branches: ["master"]
    paths: ["docs/**"]  # Direct documentation changes
```
...
**Problem Solved:** GitHub Actions doesn't trigger workflows on commits made by `GITHUB_TOKEN` (used in release workflows) to prevent infinite loops. This caused documentation workflow to never trigger after release workflow commits that updated documentation files.
...
The documentation workflow includes an intelligent **guard job** that:
...
The **release workflow** (`release.yml`) has been enhanced to support this pattern:

#### DOCS_UPDATED Flag Generation

During the version bump process, the release workflow:

1. **Tracks Documentation Updates**: Uses existing `DOCS_UPDATED` flag from bump step logic
2. **Updates Version References**: Automatically updates documentation files that reference the package version:
   - `docs/contents/development/ci-cd.mdx`
   - `docs/contents/document/installation.md`
   - `docs/contents/document/README.md`

3. **Sets Output Flag**: Outputs the `DOCS_UPDATED` flag for downstream use
```bash
# Output DOCS_UPDATED flag for documentation workflow trigger
echo "docs_updated=$DOCS_UPDATED" >> $GITHUB_OUTPUT
```

4. **Emits Artifact**: Uploads the flag as an artifact for the documentation workflow
```yaml
...
2. **Build Process**:
   - Builds static documentation with `pnpm build`
   - Generates optimized production assets

3. **GitHub Pages Deployment**:
   - Configures GitHub Pages environment
   - Uploads built documentation as Pages artifact
...
- Verify that documentation files were actually updated (check `DOCS_UPDATED` flag)
- Review the guard job output in the documentation workflow

**Guard Job Failing:**
- Ensure release workflow uploaded the `release-docs-flag` artifact
- Check that artifact download permissions are correct
- Verify workflow run ID is accessible

**Deployment Timing:**
- Workflow run triggers happen after the triggering workflow completes
- Documentation deployment will occur ~1-2 minutes after release completion
