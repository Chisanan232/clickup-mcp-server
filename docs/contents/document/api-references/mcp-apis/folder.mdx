...
```js
const es = new EventSource("http://localhost:55030/mcp/mcp/sse?tool=folder.list_in_space&space_id=SPACE_ID");
es.onmessage = (e) => {
...
curl -N -H "Content-Type: application/json" \
  -d '{"tool":"folder.list_in_space","arguments":{"space_id":"SPACE_ID"}}' \
...
async def main():
    async with ClientSession("http://localhost:55030") as s:
        result = await s.call_tool("folder.list_in_space", {"space_id": "SPACE_ID"})
        print(result)
