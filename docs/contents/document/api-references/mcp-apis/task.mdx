...
```js
const params = new URLSearchParams({ tool: 'task.create', list_id: 'LIST_ID', name: 'Ship v1.0' });
const es = new EventSource(`http://localhost:55030/mcp/mcp/sse?${params.toString()}`);
es.onmessage = (e) => {
  const frame = JSON.parse(e.data);
  if (frame.ok) console.log(frame.result);
};
```
...
curl -N -H "Content-Type: application/json" \
  -d '{"tool":"task.create","arguments":{"list_id":"LIST_ID","name":"Ship v1.0"}}' \
...
async def main():
    async with ClientSession("http://localhost:55030") as s:
        result = await s.call_tool("task.create", {"list_id": "LIST_ID", "name": "Ship v1.0"})
        print(result)
...
```js
const es = new EventSource("http://localhost:55030/mcp/mcp/sse?tool=task.get&task_id=TASK_ID");
es.onmessage = (e) => {
  const frame = JSON.parse(e.data);
  if (frame.ok) console.log(frame.result);
};
...
```bash
curl -N -H "Content-Type: application/json" \
  -d '{"tool":"task.get","arguments":{"task_id":"TASK_ID"}}' \
  http://localhost:55030/mcp/mcp
```
...
async def main():
    async with ClientSession("http://localhost:55030") as s:
        result = await s.call_tool("task.get", {"task_id": "TASK_ID"})
        print(result)
...
```js
const es = new EventSource("http://localhost:55030/mcp/mcp/sse?tool=task.list_in_list&list_id=LIST_ID&limit=50");
es.onmessage = (e) => {
  const frame = JSON.parse(e.data);
...
```bash
curl -N -H "Content-Type: application/json" \
  -d '{"tool":"task.list_in_list","arguments":{"list_id":"LIST_ID","limit":50}}' \
  http://localhost:55030/mcp/mcp
```
...
async def main():
    async with ClientSession("http://localhost:55030") as s:
        result = await s.call_tool("task.list_in_list", {"list_id": "LIST_ID", "limit": 50})
        print(result)
