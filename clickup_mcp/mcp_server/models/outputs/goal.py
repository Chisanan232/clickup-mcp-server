...
class GoalResult(BaseModel):
    ...
    key_results: List[str] = Field(
        default_factory=list, description="List of key result names", examples=[["KR1", "KR2"]]
    )
    owners: List[str] = Field(
        default_factory=list, description="List of user IDs who own this goal", examples=[["user_1", "user_2"]]
    )
