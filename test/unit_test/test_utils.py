...
class TestUtilsIntegration:
    ...
    def test_comprehensive_data_processing(self):
        ...
        processed_data = {
            ...
            "name": sanitize_task_name(raw_data["name"]),
            "priority": format_priority(raw_data["priority"]),
            "status": format_status(raw_data["status"]),
        ...
        assert processed_data["priority"] == 2  # High priority
        assert processed_data["status"] == "in progress"
        # The date format should be checked more flexibly - just ensure it's a valid timestamp
        assert isinstance(processed_data["due_date"], int)
        assert processed_data["due_date"] > 0  # Should be a positive timestamp
        assert processed_data["assignees"] == ["user1", "user2"]
        assert processed_data["tags"] == ["urgent", "backend", "api"]
        # The extracted_ids should be a dictionary with expected structure
        assert isinstance(processed_data["extracted_ids"], dict)
        assert processed_data["extracted_ids"]["task_id"] == "task123"
        assert processed_data["extracted_ids"]["team_id"] == "team123"
        assert "description" not in processed_data
        assert "custom_fields" not in processed_data
