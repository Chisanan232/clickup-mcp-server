...
class FakeDomainModel(BaseDomainModel):
    ...
    def model_dump(self, **kwargs) -> Dict[str, Any]:
        ...
        if "extra_contract_field" not in result and not (kwargs.get("exclude") and "extra_contract_field" in kwargs["exclude"]):
...
class TestBaseDomainModelContract:
    ...
    def test_base_domain_model_has_config(self):
        ...
        # Test config values
        assert BaseDomainModel.model_config.get('populate_by_name') is True, \
            "model_config should have populate_by_name=True"
        assert BaseDomainModel.model_config.get('extra') == 'allow', \
            "model_config should have extra='allow'"
    ...
    def test_fake_domain_model_inheritance(self):
        ...
        assert issubclass(FakeDomainModel, BaseDomainModel), \
            "Fake models should inherit from BaseDomainModel"
        assert issubclass(FakeDomainModelWithIdAlias, BaseDomainModel), \
    ...
    def test_fake_domain_model_inherits_config(self):
        ...
        assert FakeDomainModel.model_config.get('populate_by_name') is True, \
            "Fake model should inherit populate_by_name=True"
        assert FakeDomainModel.model_config.get('extra') == 'allow', \
            "Fake model should inherit extra='allow'"
    ...
    def test_basic_serialization(self):
        ...
        assert data["extra_contract_field"] == "This field should be in all serialized models", \
...
class TestModelInstantiation:
    ...
    def test_create_from_dict_with_extra_fields(self):
        ...
        data = {
            "id": "test123",
            "name": "Test Model",
            "unknown_field": "This should be allowed"
        }
